🎉Register now for the Data Intelligence Competition, challenge the AGI4S frontier, and win millions in prizes

🎉Precise Invoice Extraction, Efficient Enterprise. Intelligent Document Extraction in Beta

🎉Register now for the Data Intelligence Competition, challenge the AGI4S frontier, and win millions in prizes

![logo](https://webpub.shlab.tech/dps/mineru/mineru-seo-fe/mineru-seo-prod.113/_next/static/media/logo.f876e2bb.svg)
...
![mineru.docs.tab.smartParsing](https://webpub.shlab.tech/dps/mineru/mineru-seo-fe/mineru-seo-prod.113/_next/static/media/mineru-docs-smart-parsing.ddd36c07.svg)Smart Parsing
...
![mineru.docs.tab.keyExtraction](https://webpub.shlab.tech/dps/mineru/mineru-seo-fe/mineru-seo-prod.113/_next/static/media/mineru-docs-key-extraction.ddaf3128.svg)Document Smart Extraction

SDK

KIE Usage

Notice: The original v2 and v3 API versions were discontinued on 2025-01-17. Please use the v4 API under the new domain and create new tokens
...
| Supported formats | PDF, Image(png/jpg/jpeg/jp2/webp/gif/bmp), Doc, Docx, Ppt, Pptx |
...
Used for creating extract tasks via API. Users must apply for a Token first.
Notes:

- Maximum file size is 200MB, maximum page count is 600 pages
- Each account has a daily quota of 2,000 pages with highest priority; pages beyond 2,000 will have lower priority
...
**Python Example (for PDF, Doc, PPT, image files):**
...
**CURL Example (for PDF, Doc, PPT, image files):**
...
| --- | --- | --- | --- | --- |
| url | string | Yes | [https://static.openxlab.org.cn/\<br>\<br>opendatalab/pdf/demo.pdf](https://cdn-mineru.openxlab.org.cn/demo/example.pdf) | File URL, supports .pdf, .doc, .docx, .ppt, .pptx, Image(png/jpg/jpeg/jp2/webp/gif/bmp), .html formats |
...
| page\_ranges | string | No | 1-600 | Page range, formatted as a comma-separated string. E.g.: "2,4-6" selects page 2 and pages 4 to 6 (inclusive, result: \[2,4,5,6\]); "2--2" selects from page 2 to the second-to-last page ("-2" means second-to-last page). |
...
| data.full\_zip\_url | string | [https://cdn-mineru.openxlab.org.cn/](https://cdn-mineru.openxlab.org.cn/)<br>pdf/018e53ad-d4f1-475d-b380-36bf24db9914.zip | Extract result zip archive.<br>For non-HTML files, see: [https://opendatalab.github.io/MinerU/reference/output\_files/](https://opendatalab.github.io/MinerU/reference/output_files/) where layout.json corresponds to middle.json, \*\*\_model.json corresponds to model.json, \*\*\_content\_list.json corresponds to content\_list.json, and full.md is the Markdown result.<br>For HTML files: full.md is the Markdown result, main.html is the extracted main content HTML |
...
| file.page\_ranges | string | No | 1-600 | Page range, formatted as a comma-separated string. E.g.: "2,4-6" selects page 2 and pages 4 to 6 (inclusive, result: \[2,4,5,6\]); "2--2" selects from page 2 to the second-to-last page ("-2" means second-to-last page). |
| callback | string | No | [http://127.0.0.1/callback](http://127.0.0.1/callback) | URL for callback notification of extract results. Supports HTTP and HTTPS protocols. If empty, you must poll for results periodically. The callback endpoint must support POST method, UTF-8 encoding, Content-Type:application/json, and parameters checksum and content.<br>checksum: String format, generated by SHA256 algorithm from the concatenation of user uid + seed + content. User UID can be found in the personal center. To prevent tampering, you can generate the string using the above algorithm when receiving push results and verify it against the checksum.<br>content: JSON string format, parse it yourself into a JSON object. For content result examples, refer to the task query result response, corresponding to the data part of the task query result.<br>Note: After your server callback endpoint receives the extract result push from MinerU, if it returns HTTP status code 200, it indicates successful reception. Any other HTTP status code is considered a failure. On failure, MinerU will retry pushing up to 5 times until successful. If still unsuccessful after 5 retries, no further pushes will be made. Please check your callback endpoint status. |
| seed | string | No | abc\*\* | Random string used for signing callback notification requests. Consists of letters, numbers, and underscores (\_), no more than 64 characters. Defined by you, used to verify that callback notifications are initiated by the MinerU extract service.<br>Note: This field is required when using callback. |
| extra\_formats | \[string\] | No | \["docx","html"\] | Markdown and JSON are default export formats and do not need to be set. This parameter only supports one or more of docx, html, and latex formats. Not applicable to HTML source files. |
| model\_version | string | No | vlm | MinerU model version, three options: pipeline, vlm, MinerU-HTML. Default is pipeline. For HTML files, model\_version must be explicitly set to MinerU-HTML; for non-HTML files, choose pipeline or vlm |
...
| -60002 | Failed to match file format | File type detection failed. Ensure the file name and URL contain the correct extension, and the file is one of: pdf, doc, docx, ppt, pptx, png, jp(e)g |
...
## Agent Lightweight Extract Endpoints
...
| Supported File Types | PDF, Images (png/jpg/jpeg/jp2/webp/gif/bmp), Docx, PPTx, xls, xlsx |
...
| --- | --- | --- | --- |
| url | string | Required | Remote file URL, supports PDF, images, Doc/Docx, PPT/PPTx formats. HTML is not supported. |
...
| enable\_table | bool | Optional | true |
| is\_ocr | bool | Optional | false |
| enable\_formula | bool | Optional | true |
...
\| language \| string \| Optional \| Extract language, affects OCR recognition. Default `ch`. See [Language Value Reference](https://mineru.net/apiManage/docs#language-value-reference) for supported values. Only effective for PDF files \|
\| enable\_table \| bool \| Optional \| true \| Whether to enable table recognition. Default `true`. Only effective for PDF files \|
\| is\_ocr \| bool \| Optional \| false \| Whether to enable OCR. Default `false`. Only effective for PDF files \|
\| enable\_formula \| bool \| Optional \| true \| Whether to enable formula recognition. Default `true`. Only effective for PDF files \|
\| page\_range \| string \| Optional \| Page range, only effective for PDF. Supports `from-to` (e.g. `1-10`) or single page (e.g. `5`). Comma-separated complex formats are not supported. \|
...
- [Mode Comparison](https://mineru.net/apiManage/docs#mode-comparison "Mode Comparison")
- [Overview](https://mineru.net/apiManage/docs#overview "Overview")
- [Single File Extract](https://mineru.net/apiManage/docs#single-file-extract "Single File Extract")
- [Create Extract Task](https://mineru.net/apiManage/docs#create-extract-task "Create Extract Task")
- [Get Task Result](https://mineru.net/apiManage/docs#get-task-result "Get Task Result")
- [Batch File Extract](https://mineru.net/apiManage/docs#batch-file-extract "Batch File Extract")
- [Local batch File Upload Extract](https://mineru.net/apiManage/docs#local-batch-file-upload-extract "Local batch File Upload Extract")
- [Batch URL Upload Extract](https://mineru.net/apiManage/docs#batch-url-upload-extract "Batch URL Upload Extract")
- [Batch Get Task Results](https://mineru.net/apiManage/docs#batch-get-task-results "Batch Get Task Results")
- [Common Error Codes](https://mineru.net/apiManage/docs#common-error-codes "Common Error Codes")
- [Agent Lightweight Extract Endpoints](https://mineru.net/apiManage/docs#agent-lightweight-extract-endpoints "Agent Lightweight Extract Endpoints")
- [Overview](https://mineru.net/apiManage/docs#overview-1 "Overview")
- [1\. URL Extract Endpoint](https://mineru.net/apiManage/docs#1-url-extract-endpoint "1. URL Extract Endpoint")
- [2\. Local file Upload Endpoint (Signed Upload)](https://mineru.net/apiManage/docs#2-local-file-upload-endpoint-signed-upload "2. Local file Upload Endpoint (Signed Upload)")
- [3\. Query Extract Result](https://mineru.net/apiManage/docs#3-query-extract-result "3. Query Extract Result")
- [Complete Usage Examples (Python)](https://mineru.net/apiManage/docs#complete-usage-examples-python "Complete Usage Examples (Python)")
- [Agent-Specific Error Codes](https://mineru.net/apiManage/docs#agent-specific-error-codes "Agent-Specific Error Codes")
- [Language Value Reference](https://mineru.net/apiManage/docs#language-value-reference "Language Value Reference")

![Description](https://webpub.shlab.tech/dps/mineru/mineru-seo-fe/mineru-seo-prod.113/_next/static/media/logo.f876e2bb.svg)
...
![Contact Us](https://mineru.net/_next/image?url=https%3A%2F%2Fwebpub.shlab.tech%2Fdps%2Fmineru%2Fmineru-seo-fe%2Fmineru-seo-prod.113%2F_next%2Fstatic%2Fmedia%2FqrCode-1.144cd067.png&w=256&q=75)
