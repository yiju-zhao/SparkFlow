...
SEARCH_K = 350  # embedding pre-filter size
...
XINFERENCE_MODEL = os.getenv("XINFERENCE_MODEL", "Qwen3-Instruct")
...
for bu_name, query_text in bu_queries.items():
    ...
    top_sessions = top_sessions.sem_map(
    ...
    # 1. 找到 LOTUS 生成的推荐理由列名
    reason_col = [c for c in top_sessions.columns if "recommendation_reason" in c][0]

    # 2. 重命名该列为简洁的名称
    top_sessions = top_sessions.rename(columns={reason_col: "recommendation_reason"})

    # 3. 重新排列列顺序：将 rank, session_id, recommendation_reason 放在最前面
    # 其余所有原始列（title, abstract, industry 等）紧随其后
    cols = top_sessions.columns.tolist()
    # 先把我们关心的排序列提出来
    priority_cols = ["rank", "session_id", "recommendation_reason"]
    # 剩下的列（过滤掉已经在 priority_cols 里的）
    remaining_cols = [c for c in cols if c not in priority_cols]

    # 合并成最终的输出顺序
    final_col_order = priority_cols + remaining_cols
    output_df = top_sessions[final_col_order]
    ...
    print(f"    → {len(output_df)} rows, columns included: {len(output_df.columns)}")
