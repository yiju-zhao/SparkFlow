...
async def _semops_rank_impl(
    *, candidates, query, top_k, provider, model, api_key, api_base=None
) -> dict[str, Any]:
    normalized = []
    for c in candidates:
        if "match_text" in c:
...
async def _callback_impl(
    section_id: str, status: str, *, items=None, model_used=None, error=None, completed_at=None
) -> None:
...
@task
async def semops_rank(
    candidates, query_text, top_k, provider, model, api_key, api_base=None
) -> dict[str, Any]:
...
def _to_digest_items(
    pool: list[dict[str, Any]], ranked_result: dict[str, Any]
) -> list[dict[str, Any]]:
...
async def generate_section(req: GenerateSectionRequest) -> None:
    ...
    semops_candidates = [
        {"id": a["id"], "text": _build_candidate_text(a)} for a in pool if "id" in a
    ]
