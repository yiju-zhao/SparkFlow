...
Stateless w.r.t. the request body, but the workflow itself reads the
existing notebook graph + community pages directly from Postgres
(workflows.wiki_ingest._load_state). The Node-side worker still owns
per-job persistence (status writes, prisma.$transaction).
"""
...
from workflows.wiki_ingest import (
    WikiExtractRequest as InternalReq,
)
from workflows.wiki_ingest import (
    extract_wiki,
)
...
def _to_internal(req: WikiExtractRequest) -> InternalReq:
    ...
    lm = {
        "provider": req.byok.provider,
        "model": req.byok.model,
        "api_key": req.byok.apiKey,
        "api_base": req.byok.baseUrl,
    }
    if isinstance(req, WikiExtractMode):
        return InternalReq(
            ...
            source_content=req.sourceContent,
            existing_node_labels=req.existingNodeLabels,
            source_map=req.sourceMap,
            lm=lm,
        )
    return InternalReq(
        mode="remove",
...
async def extract(req: WikiExtractRequest):
    ...
    return {
        ...
        "communityPages": [
            {"slug": p.slug, "title": p.title, "markdown": p.markdown, "sourceIds": p.source_ids}
