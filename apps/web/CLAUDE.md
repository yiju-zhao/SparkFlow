...
│   │   #   (knowledge graph extraction + Louvain clustering moved to apps/langgraph/workflows/wiki_ingest.py)
...
- **Transactional commit**: `lib/services/wiki-ingest.ts::ingestSourceToWiki` POSTs to `/v1/workflows/wiki/extract` (Python implementation in `apps/langgraph/workflows/wiki_ingest.py` runs the LLM extraction + Louvain clustering + page generation), then opens one `prisma.$transaction` (`maxWait: 10s, timeout: 60s`) on the response that upserts the graph + every wiki page + deletes orphaned `community-*` slugs + appends the log entry — all-or-nothing.
