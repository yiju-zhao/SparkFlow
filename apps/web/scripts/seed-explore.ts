...
async function main() {
  ...
  const cvpr2024 = await prisma.instance.create({
    data: {
      ...
      summary:
        "The premier annual computer vision event comprising the main conference and several co-located workshops and short courses.",
    },
  });
  ...
  const pub1 = await prisma.publication.create({
    data: {
      ...
      abstract:
        "We present a novel approach to dense prediction using vision transformers. Our method achieves state-of-the-art results on semantic segmentation and depth estimation benchmarks.",
      summary:
        "A breakthrough in applying transformer architectures to pixel-level prediction tasks.",
  ...
  const pub2 = await prisma.publication.create({
    data: {
      ...
      abstract:
        "A comprehensive study of self-supervised learning methods for video understanding tasks including action recognition and temporal localization.",
  ...
  const pub3 = await prisma.publication.create({
    data: {
      ...
      abstract:
        "We extend neural radiance fields to handle dynamic scenes with moving objects and changing lighting conditions.",
    ...
    data: [
      {
        ...
        abstract:
          "We investigate the scaling behavior of large language models and derive new scaling laws that accurately predict model performance.",
      ...
      },
      {
        ...
        abstract:
          "Novel attention mechanisms that scale linearly with sequence length while maintaining model quality.",
    ...
    data: [
      {
        ...
        abstract:
          "A unified framework for understanding and improving contrastive representation learning methods.",
  ...
  const session1 = await prisma.conferenceSession.create({
    data: {
      ...
      location: "Hall A",
      abstract:
        "Presentations on the latest advances in vision transformers and their applications.",
    },
  });
  ...
  await prisma.conferenceSession.create({
    data: {
      ...
      location: "Room 201",
      overview:
        "A full-day workshop on 3D vision, neural rendering, and their applications in AR/VR.",
    },
  });
