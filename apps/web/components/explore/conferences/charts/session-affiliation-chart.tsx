...
interface SessionAffiliationChartProps {
  data: { affiliation: string; count: number }[];
}

export function SessionAffiliationChart({ data }: SessionAffiliationChartProps) {
  const option = useMemo<EChartsOption>(() => {
    if (!data || data.length === 0) return {};

    const sorted = [...data].sort((a, b) => a.count - b.count).slice(-10);
    const names = sorted.map((d) =>
      d.affiliation.length > 25 ? `${d.affiliation.substring(0, 25)}...` : d.affiliation,
    );
    ...
    return {
      tooltip: {
        ...
        formatter: (params: TooltipParams) => {
          ...
          return `${sorted[idx].affiliation}<br/>Sessions: <strong>${arr[0].value}</strong>`;
        },
      },
